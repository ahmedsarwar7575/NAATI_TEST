...
export const getAllDialogueTimeByUser = async (req, res, next) => {
  try {
    ...
    const rows = await MockTestAttempts.findAll({
      where: { userId, dialogueId: { [Op.ne]: null } },
      attributes: [
        "dialogueId",
        [sequelize.fn("SUM", sequelize.fn("COALESCE", sequelize.col("completedSeconds"), 0)), "completedSeconds"],
      ],
      group: ["dialogueId"],
      raw: true,
    });
    ...
    const totalSeconds = dialogues.reduce((sum, d) => sum + (Number(d.completedSeconds) || 0), 0);
...
export const getOneDialogueTimeByUser = async (req, res, next) => {
  try {
    ...
    if (!userId) return res.status(400).json({ success: false, message: "userId is required" });
    if (!dialogueId) return res.status(400).json({ success: false, message: "dialogueId is required" });

    const row = await MockTestAttempts.findOne({
...
export const incrementDialogueSeconds = async (req, res, next) => {
  ...
  try {
    ...
    if (!dialogueId) {
      await t.rollback();
      return res.status(400).json({ success: false, message: "dialogueId is required" });
    }
    if (!seconds) {
      await t.rollback();
    ...
    const latest = await MockTestAttempts.findOne({
      where: { userId, dialogueId },
    ...
    if (!latest) {
      await t.rollback();
      return res.status(404).json({ success: false, message: "No attempt found for this user/dialogue" });
    }

    await MockTestAttempts.update(
      { completedSeconds: sequelize.literal(`COALESCE(completedSeconds,0) + ${seconds}`) },
      { where: { id: latest.id }, transaction: t }
    );

    const sumRow = await MockTestAttempts.findOne({
      where: { userId, dialogueId },
      attributes: [
        "dialogueId",
        [sequelize.fn("SUM", sequelize.fn("COALESCE", sequelize.col("completedSeconds"), 0)), "completedSeconds"],
