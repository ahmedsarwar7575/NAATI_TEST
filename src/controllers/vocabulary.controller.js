...
export const createVocabulary = async (req, res, next) => {
  try {
    ...
    if (!originalWord || typeof originalWord !== "string" || !originalWord.trim()) {
      return res.status(400).json({ success: false, message: "originalWord is required" });
    }
    if (!convertedWord || typeof convertedWord !== "string" || !convertedWord.trim()) {
      return res.status(400).json({ success: false, message: "convertedWord is required" });
    }
    ...
    if (originalAudioUrl !== row.originalAudioUrl || convertedAudioUrl !== row.convertedAudioUrl) {
...
export const getVocabularyById = async (req, res, next) => {
  try {
    ...
    if (!row) return res.status(404).json({ success: false, message: "Vocabulary not found" });
...
export const updateVocabulary = async (req, res, next) => {
  try {
    ...
    if (languageIdRaw !== undefined) {
      ...
      if (!languageId) return res.status(400).json({ success: false, message: "Invalid languageId" });
      row.languageId = languageId;
    }

    if (originalWordRaw !== undefined) {
      if (typeof originalWordRaw !== "string" || !originalWordRaw.trim()) {
        return res.status(400).json({ success: false, message: "originalWord must be non-empty" });
      }
    ...
    if (convertedWordRaw !== undefined) {
      if (typeof convertedWordRaw !== "string" || !convertedWordRaw.trim()) {
        return res.status(400).json({ success: false, message: "convertedWord must be non-empty" });
      }
      row.convertedWord = convertedWordRaw.trim();
    }

    if (descriptionRaw !== undefined) {
      row.description = typeof descriptionRaw === "string" ? descriptionRaw.trim() : null;
    }
