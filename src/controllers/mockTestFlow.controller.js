...
// distribute TOTAL marks across N segments with 2-decimal rounding
...
const buildNotesFromResults = (results, segMap) => {
  return (results || [])
    .map((r) => {
      ...
      return [
        `SegmentId=${r.segmentId} order=${seg?.segmentOrder ?? "?"} dialogueId=${seg?.dialogueId ?? "?"}`,
        prompt ? `Prompt: ${prompt}${prompt.length >= 80 ? "..." : ""}` : "",
        `Feedback: ${line || "(none)"}`,
        `Marks: max=${r.maxMarks ?? "-"} obtained=${r.obtainedMarks ?? "-"}`,
        `Scores: accuracy=${r.accuracyScore ?? "-"} language=${r.languageQualityScore ?? "-"} fluency=${r.fluencyPronunciationScore ?? "-"} delivery=${r.deliveryCoherenceScore ?? "-"} cultural=${r.culturalControlScore ?? "-"} response=${r.responseManagementScore ?? "-"} final=${r.finalScore ?? "-"}`,
      ]
        .filter(Boolean)
        .join("\n");
    })
...
/* ============================================================
   API 0: START MOCK TEST
   - Creates MockTestSession
   - Creates MockTestResult rows for ALL segments as PENDING
   - Distributes marks: 45 per dialogue => total 90
   ============================================================ */
...
export const startMockTest = async (req, res, next) => {
  ...
  try {
    ...
    // ✅ distribute marks: 45 for dialogue1, 45 for dialogue2
...
/* ============================================================
   API 1: GET SESSION PROGRESS
   - counts how many segments completed vs pending
   - returns pending segment ids + next segment
   ============================================================ */
...
export const getMockTestProgress = async (req, res, next) => {
  try {
    ...
    // ✅ get full rows + segment + dialogue
    const results = await MockTestResult.findAll({
      ...
      // ✅ order by dialogue then segmentOrder (better than segmentId)
      order: [
    ...
    return res.json({
      ...
      // ✅ full objects (result row + nested segment + nested dialogue)
      completedSegments,
      pendingSegments,

      // ✅ full next segment object
      nextSegment,
    });
  } catch (e) {
    next(e);
...
/* ============================================================
   API 2: SUBMIT + ANALYZE ONE SEGMENT
   - creates MockTestAttempts (history)
   - updates MockTestResult for that segment -> completed + obtainedMarks
   ============================================================ */
...
export const submitMockTestSegment = async (req, res, next) => {
  ...
  try {
    ...
    // update result row (single truth for completion/progress)
...
/* ============================================================
   API 3: COMPUTE FINAL RESULT (ONLY IF ALL COMPLETED)
   - totalScore out of 90
   - pass if total>=63 AND each dialogue>=31 out of 45
...
export const computeMockTestFinalResult = async (req, res, next) => {
  ...
  try {
    ...
    // fetch segments to know dialogueId per segment
    ...
    // ✅ required: total>=63 AND each dialogue>=31 out of 45
    ...
    // ✅ SAVE FINAL RESULT IN DB (UPSERT)
    const payload = {
      mockTestSessionId: session.id,
      mockTestId: mockTest.id,
      userId,
    ...
    return res.json({
      success: true,
      session,
      finalResult: savedFinalResult, // ✅ DB row
      summary: {
        totalScore,
        outOf: 90,
        passMarks,
